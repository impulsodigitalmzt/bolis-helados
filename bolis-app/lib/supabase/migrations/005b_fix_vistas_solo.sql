-- Parche completo si 005 falló a medias: funciones + vistas + venta POS
...
SET
  precio_venta_unitario = COALESCE(v.precio_venta_unitario, s.precio_venta),
  costo_produccion_unitario = COALESCE(v.costo_produccion_unitario, s.costo_produccion_unitario)
FROM public.sabores s
WHERE v.sabor_id = s.id;

DO $$
BEGIN
  ALTER TABLE public.ventas ALTER COLUMN precio_venta_unitario SET NOT NULL;
EXCEPTION WHEN OTHERS THEN NULL;
END $$;

DO $$
BEGIN
  ALTER TABLE public.ventas ALTER COLUMN costo_produccion_unitario SET NOT NULL;
EXCEPTION WHEN OTHERS THEN NULL;
END $$;

DO $$
BEGIN
  ALTER TABLE public.ventas ALTER COLUMN vendedora_id DROP NOT NULL;
EXCEPTION WHEN OTHERS THEN NULL;
END $$;

-- Función de stock (requerida por la vista inventario)
...
-- Vistas (borrar antes por cambio de columnas)
